...
import { 
  SlidersHorizontal, Search as SearchIcon, Heart, MapPin, 
...
export default function SearchResults() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToListingsStream((newListing) => {
      // Check if new listing matches current active city filter
      if (newListing.city.toLowerCase() === city.toLowerCase()) {
        // Queue it for user display
        setSseProperties((prev) => [newListing, ...prev]);
        toast((t) => (
          <div className="flex items-center space-x-2 text-xs">
            <BellRing size={16} className="text-brand-accent animate-bounce" />
            <span>
              Real-time update: <b>{newListing.title}</b> is now available!
            </span>
          </div>
        ), { duration: 5000, icon: '🔥' });
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
          ...
          <aside className="lg:col-span-1 glass-card p-6 bg-surface-raised/40 border border-white/5 space-y-6">
            ...
            <div className="flex flex-col space-y-1.5 text-left">
              ...
              <select
                ...
                className="bg-surface-raised border border-white/10 px-3 py-2 rounded-xl text-sm focus:border-brand-primary outline-none text-text-primary"
              >
          ...
          <main className="lg:col-span-3 space-y-6">
            ...
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder="Search localities, housing names, or keywords..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-surface-raised/40 hover:bg-surface-raised/60 text-text-primary placeholder:text-text-muted text-sm font-sans pl-11 pr-4 py-3 rounded-2xl border border-white/5 focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all duration-200"
              />
            </div>
